...
class UwDebtorController extends Controller
{
    public function postKatm(Request $request)
    {
        ...
        if ($post_type == 'reg') {
            ...
            $credit_history = UwPhyKatmDebFile::where('uw_deb_id', $model_id)->where('uw_katm_id', 0)->where('file_type', '=', 'B64_K_HIS')->first();
    ...
    public function onlineRegistration($model_id)
    {
        ...
        $client = UwClients::find($modelClient->uw_clients_id);

        $modelLoanType = UwLoanTypes::find($client->loan_type_id);
        ...
        $agreement_number = $client->agreement_number;
        $agreement_date = $client->claim_date;
        $claim_date     = $client->claim_date;
        $credit_type    = $client->loanType->credit_type;
        $registration_address = $client->registration_address;
        $is_inps = $client->is_inps;
        $reg_status = $modelClient->isReg;
        ...
        $data = array(
            ...
            "request" => array(
                ...
                "claim_date" => "".date("d.m.Y", strtotime($claim_date))."",
                "inn" => "".$modelClient->inn."",
                "claim_number" => "".$modelClient->claim_number."",
                "agreement_number" => "".$agreement_number."",
                "agreement_date" => "".date("d.m.Y", strtotime($agreement_date))."",
                "resident" => "1",
                "document_type" => "6",
    ...
    public function clientDecline($model_id)
    {
        ...
        $isCheckKatm = UwKatmDebClients::where('uw_deb_id', $model_id)->first();
    ...
    public function curlResultDecline($arrData, $result)
    {
        ...
        if ($code == '200') {
            ...
            if ($result == '05000') {
                ...
                // update reg
                $modelClient = UwClientDebtors::find($model_id);
                $modelClient->update(['status' => -1, 'isReg' => 0, 'katm_sir' => '']);
        ...
        if ($code == '200') {
            ...
            if ($result_code == '05000') {
                ...
                if ($models_status == 1) {
                    ...
                    $old_inps = UwInpsDebClients::where('uw_deb_id', $model_id)->where('status', 1);
                    $old_inps->delete();
